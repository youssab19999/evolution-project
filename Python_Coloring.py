...
STYLES = {
       ...
       'comment': format('light Green', 'italic'),
       'self': format('black', 'italic'),
       'numbers': format('dark grey'),
       'builtInFunctions': format('dark green'),
       'escapeCharacters': format('darkBlue')
   }
...
class PythonHighlighter(QSyntaxHighlighter):
    ...
    builtInFunctions = [
        "abs","delattr","hash","memoryview","set","all","dict","help","min","setattr","any","dir","hex",
         "next","slice","ascii","divmod","id","object","sorted","bin",
         "enumerate","input","oct","staticmethod","bool","eval","int","open","str","breakpoint",
         "exec","isinstance","ord","sum","bytearray","filter","issubclass","pow","super","bytes",
    ...
    def __init__(self, document):
        ...
        # Keyword,built in functions ,operator, and brace rules
        rules += [(r'\b%s\b' % w, 0, STYLES['keyword'])
                  for w in PythonHighlighter.keywords]
        rules += [(r'\b%s\b' % bi, 0, STYLES['builtInFunctions'])
                  for bi in PythonHighlighter.builtInFunctions]
